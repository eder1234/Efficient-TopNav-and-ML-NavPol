...
class Registration:
    ...
    def save_colored_registration_result(self, count_steps):
        if self.config['logs']['merged_pc']:
            # Define red, blue, and green colors
            red = np.array([1, 0, 0])  # RGB for red
            blue = np.array([0, 0, 1])  # RGB for blue
            green = np.array([0, 1, 0])  # RGB for green

            # Make a safe copy of pc1 to keep the initial points in green
            initial_pc1 = copy.deepcopy(self.pc1)

            # Transform pc1 according to the ICP result
            self.pc1.transform(self.icp_result.transformation)

            # Assign red color to all points in the transformed pc1
            colors_pc1_transformed = np.tile(red, (len(self.pc1.points), 1))
            self.pc1.colors = o3d.utility.Vector3dVector(colors_pc1_transformed)

            # Assign green color to all points in the initial pc1
            colors_initial_pc1 = np.tile(green, (len(initial_pc1.points), 1))
            initial_pc1.colors = o3d.utility.Vector3dVector(colors_initial_pc1)

            # Assign blue color to all points in pc2
            colors_pc2 = np.tile(blue, (len(self.pc2.points), 1))
            self.pc2.colors = o3d.utility.Vector3dVector(colors_pc2)

            # Merge the initial, transformed point clouds, and pc2
            merged_pc = initial_pc1 + self.pc1 + self.pc2
    ...
    def save_merged_and_recolored_pc(self, point_cloud, count_steps):
        ...
        pc_path = pc_dir + f"/{self.run_id:04d}_merged_pc_{count_steps:04d}.ply"  # Random ID for uniqueness

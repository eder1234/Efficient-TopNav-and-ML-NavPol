...
class PointCloud:
    def __init__(self, config, kp1=None, kp2=None, current_depth=None, target_depth=None, pc1=None, pc2=None, points1=None, points2=None, colors1=None, colors2=None, id=0):
            self.config = config
            self.kp1 = kp1
            self.kp2 = kp2
    ...
    def save_pc_as_ply(self): # Not yet implemented
        if self.config['logs']['pc']:
            #pc_dir = self.config['paths']['LOGS_DIR'] + 'point_clouds'
            pc_dir = self.config['log_pc']
            os.makedirs(pc_dir, exist_ok=True)
            print("Saving point clouds to: ", pc_dir)
            pc1_path = pc_dir + f"{self.id:04d}_pc_current_{self.count_steps:04d}.ply"
            pc2_path = pc_dir + f"{self.id:04d}_pc_target_{self.count_steps:04d}.ply"
